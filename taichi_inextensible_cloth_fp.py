...
class InextensibleClothFP:
    def __init__(self, n_u=30, n_v=30, dx=0.03, dt=1/240, iters=15, pcg_iters=60,
        ...
        self.n_u = n_u  # grid resolution along U (warp)
        self.n_v = n_v  # grid resolution along V (weft)
        self.n = n_u * n_v
        self.dx = dx
        self.dt = float(dt)  # 使用 Python 的 float 类型
        self.fp_outer_iters = iters        # outer FP linearization iterations
        self.pcg_max_iters = pcg_iters     # PCG iterations for SPD system
        self.pcg_tol = tol
        self.g = vec3(g)
        ...
        self.f = ti.Vector.field(3, dtype=real, shape=self.n)  # external forces
        self.inv_m = ti.field(dtype=real, shape=self.n)

        # Grid indexing helpers
        # idx(u,v) -> linear index
        self.U = n_u
        self.V = n_v

        # Constraints: only axis-aligned (warp/weft) edges are hard-inextensible
        # Each constraint has (i, j, l0)
        ...
        # Per-constraint residual C(x) and gradient helpers (g_c = 2*(d)/l0)
        self.C = ti.field(dtype=real, shape=self.max_cons)
        self.g_a = ti.Vector.field(3, dtype=real, shape=self.max_cons)  # -g_c (contribution on i)
        self.g_b = ti.Vector.field(3, dtype=real, shape=self.max_cons)  # +g_c (contribution on j)

        # Variables for matrix-free CG over λ
        self.dlambda = ti.field(dtype=real, shape=self.max_cons)  # solution increment
        self.r = ti.field(dtype=real, shape=self.max_cons)        # residual
        self.p = ti.field(dtype=real, shape=self.max_cons)        # conjugate direction
        self.Ap = ti.field(dtype=real, shape=self.max_cons)       # A*p

        # Scratch per-vertex when doing J^T * z and Minv * vec
    ...
    def _build_topology(self):
        # Register horizontal and vertical constraints with rest lengths
        cons = []
        for v in range(self.V):
            for u in range(self.U):
                i = v * self.U + u
                if u + 1 < self.U:  # horizontal edge
                    j = v * self.U + (u + 1)
                    cons.append((i, j, self.dx))
                if v + 1 < self.V:  # vertical edge
                    j = (v + 1) * self.U + u
                    cons.append((i, j, self.dx))
        # Upload
    ...
    def _init_states(self):
        for v in range(self.V):
            for u in range(self.U):
                i = v * self.U + u
                # initial flat cloth (slightly lifted in y)
        ...
        # pin two top corners (set inv mass = 0)
        self.inv_m[self.idx(0, 0)] = 0.0
        self.inv_m[self.idx(self.U - 1, 0)] = 0.0
    ...
    def explicit_predict(self, dt: real):
        # x_prev for velocity update later
        for i in range(self.n):
            self.x_prev[i] = self.x[i]
        # semi-implicit Euler on external forces: v <- v + dt * Minv * f; x0 <- x + dt * v
        for i in range(self.n):
            if self.inv_m[i] > 0:
                self.v[i] += dt * self.inv_m[i] * self.f[i]
                self.x[i] += dt * self.v[i]

    # ------------ Fast Projection machinery ---------------
    @ti.kernel
    def compute_constraint_residuals_and_grads(self):
        # For current x, compute C[c] and g vectors per constraint
    ...
    def step(self):
        ...
        self.apply_ground()
        self.fast_projection()   # enforce inextensibility
